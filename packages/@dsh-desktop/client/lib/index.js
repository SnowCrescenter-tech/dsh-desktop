 ...
 */
function installTrayHandler() {
    const w = globalThis;
    w.__dshDesktopCommand__ = (command) => {
 ...
 * 暴露 Web → 原生 通知助手到全局。
 */
function installNotifyHelper(bridge) {
    const notify = bridge.native?.notify;
    const w = globalThis;
    w.dshDesktopNotify = (payload) => notify ? notify(payload) : Promise.resolve();
    return () => {
